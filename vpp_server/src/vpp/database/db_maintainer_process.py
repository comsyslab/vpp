...
class DBMaintainerProcess(AbstractVPPProcess):
    ...
    def start(self):
        ...
        period_days = ConfigIniParser().get_rolling_window_length()
        self.period_seconds = int(datetime.timedelta(days=period_days).total_seconds())
    ...
    def run_maintenance_task(self):
        ...
        time_to_next_run = self.period_seconds - time_spent
    ...
    def _do_maintenance(self):
        timestamp = datetime.datetime.now(tz=tzlocal.get_localzone()) - datetime.timedelta(seconds=self.period_seconds)
