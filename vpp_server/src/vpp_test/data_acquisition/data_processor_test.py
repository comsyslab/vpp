...
class DefaultDataProcessorTest(unittest.TestCase):
    ...
    def test_default_data_processor_measurements(self):
        ...
        self.assertEqual(db_manager.data_dicts, None)

        data_processor = DefaultMeasurementProcessingStrategy()
        data_processor.process_data(data, db_manager=db_manager)
        self.assertEqual(len(db_manager.data_dicts), 1)

        self.assertEqual(db_manager.data_dicts[0]['sensor_id'], sensor_id)
        self.assertEqual(db_manager.data_dicts[0]['timestamp'], timestamp)
        self.assertEqual(db_manager.data_dicts[0]['value'], value)
    ...
    def test_default_prediction_processor_predictions(self):
        ...
        self.assertEqual(len(db_manager.data_dicts), 1)
        pred_dict = db_manager.data_dicts[0]
        self.assertEqual(pred_dict, pred)
...
class DBManagerStub():
    ...
    def store_new_data_bulk(self, dicts):
